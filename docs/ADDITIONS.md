...
| 5.4 | **Revision round manager** — feedback attached to an element, then a version, then approval | **PART** | Two separate things, now separated. `client_portal_review_rounds` attaches a client's answer to a specific version (Phase 6); `lib/revisions.js` stays the billing/count record. Feedback is not yet attached to an element *within* a direction. |
| 5.5 | **Approval creates a durable record** | **BUILT** | Phase 6. `client_portal_review_responses` is append-only — every answer is kept, the current verdict is the latest, and each names the exact version it answered. `step_status` is now a derived mirror, and the completeness check reads the one source rather than three local fields that were never client approvals. |
| 5.6 | **"Client said yes" — approval as a real milestone that moves the project on** | **PART** | Phase 6 states it plainly on the share panel ("Your client approved the identity on …"). It deliberately does not move the project on: nothing unlocks, gates, or advances. |
| 5.7 | **Client presentation builder** — present concepts before approval, not just the final book | **PART** | Phase 5 built the studio half; Phase 6 added the client half — the frozen Presentation Version is shown in the portal and collects feedback plus a preferred direction. It collects no approval, by design: a shortlist is not a thing to say yes to. |

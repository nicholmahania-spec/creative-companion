...
import {
  contrastRatio,
  formatRatio,
  /* `logoInk` is sampled from the rasterised mark by this file, so it is the
     one colour value `buildBookStyle` cannot hand over already normalised.
     A pure utility from the shared colour module, not a second colour
     authority — `renderContext` reads it from here too. */
  normalizeHex,
  tintsAndShades,
} from '../color'
import {
...
export async function downloadBrandPackVectorPdf(
  ...
  try {
    ...
    const {
      ...
      colorSys,
      inkHex,
      /* The accent hex, not just its rgb triple. `drawMarkOrMonogram` asks
         "would the mark be visible on this ground?", and `contrastRatio` takes
         hex. 10B moved the derivation into `buildBookStyle`, which returns
         this alongside `GOLD` — so it is taken from the style like every other
         colour rather than recomputed here. */
      goldHex,
      creamHex,
      INK,
      GOLD,

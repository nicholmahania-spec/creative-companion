    ...
    <>
          <div className="brand-layout surface-document system-view design-studio view-enter" data-nav-dir={navDir}>
            ...
            >
              ...
              {/* Logo Concepts Strip - displays multiple concepts for client selection */}
              {activeProject?.logoConcepts && (
                <div className="logo-concepts-section" style={{ margin: '1.5rem 0' }}>
                  <div className="field-block" style={{ marginBottom: '0.75rem' }}>
                    <label className="field-label">Logo Concepts</label>
                    <p className="field-hint" style={{ margin: '0.25rem 0 0.5rem' }}>
                      Add 2-3 logo concepts, then ★ one as the chosen direction
                    </p>
                    <div className="logo-concepts-strip" style={{ display: 'flex', gap: '0.75rem', overflowX: 'auto', padding: '0.5rem 0' }}>
                      {activeProject.logoConcepts.map((concept) => (
                        <div
                          key={concept.id}
                          className={`logo-concept-card${concept.chosen ? ' is-chosen' : ''}`}
                          style={{
                            flex: '0 0 120px',
                            textAlign: 'center',
                            cursor: 'pointer',
                            border: concept.chosen ? '2px solid var(--dopamine)' : '1px dashed var(--field-line)',
                            borderRadius: 'var(--radius)',
                            padding: '0.5rem',
                            backgroundColor: concept.chosen ? 'rgba(16, 185, 129, 0.1)' : 'transparent'
                          }}
                          onClick={() => {
                            // Toggle selection when clicking on a concept
                            if (concept.chosen) {
                              // If clicking on already chosen concept, unselect it
                              updateLogoConcept(concept.id, { chosen: false });
                              setLogoConceptChosen(''); // Clear selection
                            } else {
                              // Select this concept
                              setLogoConceptChosen(concept.id);
                            }
                          }}
                        >
                          {/* Concept thumbnail */}
                          ...
                          <div style={{ fontSize: '0.75rem', fontWeight: '500', marginBottom: '0.25rem' }}>
                            Concept {['A', 'B', 'C', 'D', 'E'].find((_, i) =>
                              activeProject?.logoConcepts?.findIndex(c => c.id === concept.id) === i) ||
                              concept.id.substring(0, 4).toUpperCase()
                            }
                          </div>
                          ...
                          {/* Remove button (only show on hover or always for now) */}
                          <button
                            onClick={(e) => {
                              e.stopPropagation(); // Prevent triggering concept selection
                              removeLogoConcept(concept.id);
                            }}
                            style={{
                              marginTop: '0.25rem',
                      ...
                      <div
                        ...
                        onClick={async () => {
                          // Add a new concept and open file upload for it
                          addLogoConcept();

                          // Find the newly added concept (last one)
                          const store = useAppStore.getState();
                          const updatedProject = store.projects.find(
                            p => p.id === store.currentProjectId
                          );
                          const newConcept = updatedProject?.logoConcepts?.slice(-1)[0];

                          if (newConcept) {
                            // Open file upload for the new concept
                            ...
                            input.onchange = async (e) => {
                              ...
                              reader.onload = async () => {
                                try {
                                  ...
                                  updateLogoConcept(newConcept.id, { image: scaled });
                                } catch {
                                  updateLogoConcept(newConcept.id, { image: reader.result });
                                }
                                const bump = bumpDesignVersionIfV1();
                                flashMicro(
                                  bump?.bumped
                                    ? `Concept image · ${bump.version}`
                                    : 'Concept image added'
                                );
                              };
                    ...
                    <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginTop: '0.5rem' }}>
                      ...
                      • Click "+ Add Concept" to upload a logo idea<br/>
                      • Click on a concept to ★ select it as the chosen direction<br/>
                      • The chosen concept automatically becomes the logo used everywhere<br/>
                      • Remove concepts with the − button<br/>
                      • star, don't sort – no ranking or ordering needed
                    </div>
                  </div>
                </div>
              )}
